...
async def wait_for_selector_with_retry(page: Page, selector: str, retries: int = 3, base_delay: int = 1):
    ...
    for attempt in range(retries):
        try:
            return await page.wait_for_selector(selector, timeout=20000)
        except TimeoutError:
...
async def change_date_to_today(page: Page):
    ...
    try:
        ...
        await page.get_by_text("9:00 PM").click()
        await page.get_by_role("button", name="Apply").click()
        await page.get_by_role("columnheader", name="49").locator("div").click()
    except Exception as e:
        logger.error("Failed to change the date to today.", exc_info=True)
...
async def process_url(page: Page, url: str, retries=3) -> Optional[dict]:
    for attempt in range(retries):
        ...
        try:
            ...
            await change_date_to_today(page)
